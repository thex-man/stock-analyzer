...
def check_and_crawl_incremental():
    ...
    if all_new_records:
        print("\n[*] 以下是今日新增的问答记录，请手动标记利好：\n")
        for i, record in enumerate(all_new_records, 1):
            print(f"{i}. {record['证券简称']}（{record['证券代码']}）{record['日期']}")
            print(f"   问：{record['问题'][:80]}")
            print(f"   答：{record['回答'][:80]}")
            print()

        # 让用户标记利好
        while True:
            choice = input("是否标记利好并保存？(y:全部保存 n:跳过 q:退出): ").strip().lower()
            if choice == 'y':
                save_good_news(all_new_records)
                break
            elif choice in ('n', 'q'):
                break
